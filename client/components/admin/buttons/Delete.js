...
const Delete = ({id, name}) => {
  ...
  return (
    <div>
    <button className='btn bg-red-500 border-none p-3 m3 hover:text-red-500' onClick={() => setShowDelete(true)}>Delete {sectionType}</button>
    {showDelete && (
        ...
        <div className="bg-white p-4 rounded-md shadow-md text-black w-[calc(15vw + 300px)]">
            <h2 className='text-xl font-bold m-5'>Are you sure you want to delete {sectionType}</h2>
            <div className="flex gap-2 justify-center items-center p-5">
                <button className='btn btn-sm btn-error bg-red-500 border-none hover:text-gray-200' type="submit" onClick={deleteId}>Delete {sectionType}</button>
